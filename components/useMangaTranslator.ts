...
export default function useMangaTranslator() {
  ...
  const [textSourceLanguage, setTextSourceLanguage] =
    useState('eng')

  const [textTargetLanguage, setTextTargetLanguage] =
    useState('vi')

  const [textInput, setTextInput] = useState('')

  const [textOutput, setTextOutput] = useState('')
  const [imageToolStatus, setImageToolStatus] =
    useState('')
  const [removeBackgroundStrength, setRemoveBackgroundStrength] =
    useState(38)

  const [isTranslatingText, setIsTranslatingText] =
    useState(false)
  ...
  const [hoveredOverlayId, setHoveredOverlayId] =
    useState<string | null>(null)
  const [activeOverlayId, setActiveOverlayId] =
    useState<string | null>(null)
  const [draftSelection, setDraftSelection] =
    useState<SelectionRect | null>(null)
  const [draftOcrText, setDraftOcrText] =
    useState('')
  const [editorSentence, setEditorSentence] =
    useState('')
  const [editorTranslation, setEditorTranslation] =
    useState('')
  const [isUpdatingOverlay, setIsUpdatingOverlay] =
    useState(false)
  const interactionRef =
    useRef<PointerInteraction | null>(null)
  const generatedImageCountRef = useRef(0)

  const [overlaysByImage, setOverlaysByImage] =
    useState<
  ...
  const activeOverlay = activeOverlayId
    ? currentOverlays.find(
        (overlay) => overlay.id === activeOverlayId
      ) ?? null
    : null
  const activeEditorRegion =
    activeOverlay ?? draftSelection
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(overlaysByImage)
    )
  }, [overlaysByImage])
  ...
  useEffect(() => {
    ...
    const overlay = (
      overlaysByImage[currentImage.id] ?? []
    ).find((item) => item.id === activeOverlayId)
  ...
  const translateText = async (
    ...
    try {
      ...
      const data =
        (await response.json()) as GoogleTranslateResponse
  ...
  const recognizeOriginalRegion = async (
    originalRegion: SelectionRect
  ) => {
    ...
    const realX = Math.max(
      0,
      Math.round(originalRegion.x)
    )
    const realY = Math.max(
      0,
      Math.round(originalRegion.y)
    )
    ...
    const grayscaleCanvas =
      createGrayscaleCanvas(tempCanvas)
    const inkBounds =
      detectInkBounds(grayscaleCanvas)
    const focusedRawCanvas =
      cropCanvasToBounds(tempCanvas, inkBounds)
    const focusedProcessedCanvas =
      cropCanvasToBounds(
        processedCanvas,
        inkBounds
      )
    const focusedGrayscaleCanvas =
      cropCanvasToBounds(
        grayscaleCanvas,
        inkBounds
      )
    ...
    for (const attempt of attempts) {
      const recognitionLanguage =
        attempt.label.split(':')[0]
      const recognitionFamily =
        getLanguageFamily(recognitionLanguage)
    ...
    if (
      isCjkLanguage(sourceLanguage) &&
      (bestCandidate.scriptRatio < 0.45 ||
        bestCandidate.confidence < 20)
    ) {
      return null
    }

    const textForTranslation =
      bestCandidate.textForTranslation
    ...
    const refinedRegion = inkBounds
      ? {
          x:
            realX +
            Math.round(
              inkBounds.x / upscaleFactor
            ),
          y:
            realY +
            Math.round(
              inkBounds.y / upscaleFactor
            ),
          width: Math.round(
            inkBounds.width / upscaleFactor
          ),
          height: Math.round(
            inkBounds.height / upscaleFactor
          ),
        }
      : {
          x: realX,
  ...
  const runOCR = async (
    ...
    try {
      ...
      const recognitionResult =
        await recognizeOriginalRegion({
          x: Math.max(
            0,
            Math.round(selectedArea.x * scaleX)
          ),
          y: Math.max(
            0,
            Math.round(selectedArea.y * scaleY)
          ),
          width: Math.round(
      ...
      if (currentImage) {
        ...
        setDraftOcrText(
          recognitionResult.cleanedText
        )
        setActiveOverlayId(null)
        setEditorSentence(
          recognitionResult.textForTranslation
        )
        setEditorTranslation('')
      }
  ...
  const scanEntireImage = async () => {
    try {
      ...
      tempCanvas.width = Math.max(
        1,
        Math.round(originalImage.width * scale)
      )
      tempCanvas.height = Math.max(
        1,
        Math.round(originalImage.height * scale)
      )
      ...
      const processedCanvas =
        preprocessForFullPageManga(tempCanvas)
      ...
      const bestScan = bestCandidate
        ? scanResults.find(
            (item) =>
              item.label === bestCandidate.label
          ) ?? null
        : null
      ...
      const toOriginalRegion = (bbox: {
        ...
        return {
          x: Math.max(
            0,
            Math.round(bbox.x0 / scaleDownFactor)
          ),
          y: Math.max(
            0,
            Math.round(bbox.y0 / scaleDownFactor)
          ),
          width: Math.min(
            originalImage.width,
      ...
      const blockCandidates = bestScan.blocks
        .map((block) => {
          ...
          if (
            width <
              processedCanvas.width * 0.012 ||
            height <
              processedCanvas.height * 0.009 ||
        ...
        blockCandidates.length > 0
          ...
          : bestScan.words
              .filter((word) => {
                ...
                return (
                  width >=
                    processedCanvas.width * 0.008 &&
                  height >=
                    processedCanvas.height * 0.007
                )
              })
              .sort((a, b) => {
                const rowDiff =
                  (a.bbox.y0 ?? 0) - (b.bbox.y0 ?? 0)
              ...
              .filter(
                (item) =>
                  item.sentenceText.trim().length >= 2
              )
      ...
      const nextOverlays: TranslationOverlay[] =
      ...
      if (nextOverlays[0]) {
        ...
        setEditorSentence(
          nextOverlays[0].sentenceText
        )
        setEditorTranslation(
          nextOverlays[0].translatedText
        )
      }
  ...
  const movePage = (
    ...
    const [movedImage] = nextImages.splice(
      fromIndex,
      1
    )
  ...
  const mergeImages = async () => {
    ...
    try {
      ...
      const mergedWidth = Math.max(
        ...loadedImages.map((image) => image.width)
      )
      ...
      const mergedBlob = await new Promise<Blob | null>(
        (resolve) => {
      ...
      setOcrText(
        'Merged image created from the current Uploaded Pages order.'
      )
    } catch (error) {
      console.error(error)
  ...
  const appendGeneratedImage = async (
    ...
    if (!blob) {
      setImageToolStatus(
        'Could not generate the new image.'
      )
      return
    }
  ...
  const cropCurrentImage = async () => {
    try {
      ...
      const croppedCanvas = extractCanvasRegion(
        sourceCanvas,
        {
  ...
  const removeBackgroundFromCurrentImage =
    async () => {
      try {
  ...
  const downloadCurrentImage = async () => {
    try {
      ...
      console.error(error)
      setImageToolStatus(
        'Failed to download current image.'
      )
    }
  }
  ...
  const clampSelection = (
    ...
    const x = Math.max(
      0,
      Math.min(nextSelection.x, bounds.width - width)
    )
    const y = Math.max(
      0,
      Math.min(nextSelection.y, bounds.height - height)
    )
        ...
        clampSelection(
          {
            ...
            x:
              interaction.initialSelection.x +
              (point.x - interaction.startX),
            y:
              interaction.initialSelection.y +
              (point.y - interaction.startY),
          },
          point.bounds
        )
  ...
  const scanSelection = async () => {
    if (
      ...
      setOcrText(
        'Selection is too small. Resize the scan area and try again.'
      )
      return
    }
  ...
  const selectOverlay = (overlayId: string) => {
    const overlay = currentOverlays.find(
      ...
      [currentImage.id]: (
        ...
        overlay.id === activeOverlayId
          ? {
              ...overlay,
              sentenceText:
                nextSentence || overlay.sentenceText,
  ...
  const getOverlayStyle = (
    ...
    const translatedLength =
      overlay.translatedText.trim().length || 1

    const widthBasedFontSize =
      scaledWidth /
      Math.max(6, translatedLength * 0.62)
  ...
  const getOverlayEditorStyle = (
    ...
    const panelWidth = Math.min(
      280,
      Math.max(220, displaySize.width * 0.32)
    )
    const gap = 14
    const fitsRight =
      overlayStyle.left +
